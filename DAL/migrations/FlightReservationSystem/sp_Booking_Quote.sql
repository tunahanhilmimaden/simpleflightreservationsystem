CREATE OR ALTER PROCEDURE [FlightReservationSystem].[sp_Booking_Quote]
  @FlightId INT,
  @SeatNumbers NVARCHAR(MAX)
AS
BEGIN
  SET NOCOUNT ON;
  DECLARE @t TABLE (SeatNumber NVARCHAR(10));
  INSERT INTO @t(SeatNumber)
  SELECT LTRIM(RTRIM(value)) FROM STRING_SPLIT(@SeatNumbers, ',');
  ...
  WHERE f.FlightID = @FlightId
    AND s.SeatNumber IN (SELECT SeatNumber FROM @t);
END
